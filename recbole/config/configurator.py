...
class Config(object):
    ...
    def _init_device(self):
        ...
        if "local_rank" not in self.final_config_dict:
            ...
            # print(self.final_config_dict["device"])
            self.final_config_dict["device"] = (
                torch.device("cpu")
                if len(gpu_id) == 0 or not torch.cuda.is_available() or \
                    ("device" in self.final_config_dict.keys() and self.final_config_dict["device"] == "cpu") # edit
                else torch.device("cuda")
            )
            # print("In configurator: device = {}".format(self.final_config_dict["device"]))
            # time.sleep(1000)
        else:
