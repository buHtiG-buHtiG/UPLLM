...
class Trainer(AbstractTrainer):
    ...
    def _train_epoch(self, train_data, epoch_idx, loss_func=None, show_progress=False):
        ...
        for batch_idx, interaction in enumerate(iter_data):
            ...
            # time1 = tm.time()
            # print("before train: {}".format(time1 - allstart))
            with torch.autocast(device_type=self.device.type, enabled=self.enable_amp):
                # start = tm.time()
                # if type(self.model).__name__ == "XSimGCL":
                ...
                # tm.sleep(5)
            # end2 = tm.time()
            # print("exit autocast: {}".format(end2 - end))
            ...
            if isinstance(losses, tuple):
                ...
                # time14 = tm.time()
                # print("Detailed processing loss:")
                # print(time12 - time11)
                # print(time13 - time12)
                # print(time14 - time13)
            else:
                ...
                # print(time13 - time12)
            # time2 = tm.time()
            # print("process loss: {}".format(time2 - end2))
            if torch.isnan(loss):
                self._check_nan(loss)
            # time3 = tm.time()
            # print("check nan: {}".format(time3 - time2))
            scaler.scale(loss + sync_loss).backward()
            # time4 = tm.time()
            # print("backward: {}".format(time4 - time3))
            
            if self.clip_grad_norm:
                clip_grad_norm_(self.model.parameters(), **self.clip_grad_norm)
            scaler.step(self.optimizer)
            # time5 = tm.time()
            # print("step: {}".format(time5 - time4))
            scaler.update()
            # time6 = tm.time()
            # print("update: {}".format(time6 - time5))
            if self.gpu_available and show_progress:
                iter_data.set_postfix_str(
                    set_color("GPU RAM: " + get_gpu_usage(self.device), "yellow")
                )
            # all_end = tm.time()
            # print("total cost time: {}".format(all_end - allstart))
        total_loss = sum(total_loss_list).item()
        # all_end_end = tm.time()
        # print("sum loss time: {}".format(all_end_end - all_end))
        return total_loss
    ...
    def evaluate(
        ...
        if load_best_model:
            ...
            # print("****** load: {}".format(checkpoint['epoch']))
            self.model.load_state_dict(checkpoint["state_dict"])
            self.model.load_other_parameter(checkpoint.get("other_parameter"))
