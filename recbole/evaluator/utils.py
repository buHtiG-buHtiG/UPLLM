...
def _binary_clf_curve_preprocess(trues, preds):
    trues = trues == 1

    desc_idxs = torch.argsort(preds).flip(0)
    preds = preds[desc_idxs]
    trues = trues[desc_idxs]

    unique_val_idxs = torch.where(torch.diff(preds))[0]
    # threshold_idxs = np.r_[unique_val_idxs, trues.size - 1]
    temp = torch.LongTensor([trues.shape[0] - 1]).to(unique_val_idxs.device)
    threshold_idxs = torch.hstack([unique_val_idxs, temp])

    tps = torch.cumsum(trues, dim=-1)[threshold_idxs]
    fps = 1 + threshold_idxs - tps
    return fps, tps