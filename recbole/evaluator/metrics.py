...
from recbole.evaluator.utils import _binary_clf_curve, _binary_clf_curve_preprocess
...
class AUC(LossMetric):
    ...
    def metric_info_preprocess(self, preds, trues):
        fps, tps = _binary_clf_curve_preprocess(trues, preds)
        # if len(fps) > 2:
        if fps.shape[0] > 2:
            temp = torch.Tensor([True]).to(fps.device)
            optimal_idxs = torch.where(
                torch.hstack([temp, torch.logical_or(torch.diff(fps, 2), torch.diff(tps, 2)), temp])
                # np.r_[True, np.logical_or(np.diff(fps, 2), np.diff(tps, 2)), True]
            )[0]
            fps = fps[optimal_idxs]
            tps = tps[optimal_idxs]

        temp = torch.Tensor([0]).to(tps.device)
        tps = torch.hstack([temp, tps])
        fps = torch.hstack([temp, fps])
        # tps = np.r_[0, tps]
        # fps = np.r_[0, fps]
        ...
        if fps[-1] <= 0:
            ...
            fpr = torch.Tensor([torch.nan]).to(fps.device).repeat(fps.shape[0])
            # fpr = np.repeat(np.nan, fps.shape)
        else:
            fpr = fps / fps[-1]
        ...
        if tps[-1] <= 0:
            ...
            tpr = torch.Tensor([torch.nan]).to(tps.device).repeat(tps.shape[0])
            # tpr = np.repeat(np.nan, tps.shape)
        else:
            tpr = tps / tps[-1]

        result = self.sk_auc_preprocess(fpr, tpr)
        return result
    
    def sk_auc_preprocess(self, x, y):
        ...
        area = direction * torch.trapz(y, x).item()
