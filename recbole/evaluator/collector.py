...
class DataStruct(object):
    ...
    def update_tensor_preprocess(self, name: str, value: torch.Tensor):
        # value2 = torch.zeros(value.shape[0])
        # # print(value2.device)
        ...
        if name not in self._data_dict:
            ...
            if not isinstance(self._data_dict[name], torch.Tensor):
                raise ValueError("{} is not a tensor.".format(name))
            self._data_dict[name] = torch.cat(
                (self._data_dict[name], value), dim=0
            )
...
class Collector(object):
    ...
    def eval_batch_collect_preprocess(
        self,
        scores_tensor: torch.Tensor,
        interaction,
        positive_u: torch.Tensor,
        ...
        if self.register.need("rec.score"):
            # print("scores_tensor")
            # print(scores_tensor.device)
            self.data_struct.update_tensor_preprocess("rec.score", scores_tensor)

        # return
        if self.register.need("data.label"):
            self.label_field = self.config["LABEL_FIELD"]
            # print("interaction")
            # print(interaction[self.label_field].device)
            self.data_struct.update_tensor_preprocess(
