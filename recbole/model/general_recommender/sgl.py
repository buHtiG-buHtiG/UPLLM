...
class SGL(GeneralRecommender):
        ...
        if self.use_llm_embed:
            ...
            if self.merge_embed != 'none':
                self.model_user_embedding = torch.nn.Embedding(
                    num_embeddings=self.n_users, embedding_dim=self.embed_dim
                )
                self.model_item_embedding = torch.nn.Embedding(
                    num_embeddings=self.n_items, embedding_dim=self.embed_dim
                )
        else:
    ...
    def forward(self, graph):
        ...
        main_ego = torch.cat([user_embeddings, item_embeddings])
        all_ego = [main_ego]
        ...
        return user_emd, item_emd, cl_loss
    ...
    def calculate_loss(self, interaction):
        ...
        user_emd, item_emd, cl_loss_train = self.forward(self.train_graph)
        user_sub1, item_sub1, cl_loss1 = self.forward(self.sub_graph1)
        user_sub2, item_sub2, cl_loss2 = self.forward(self.sub_graph2)
        
        if self.use_llm_embed and self.merge_embed == 'cl-gen':
            model_all_embed = torch.concat([user_emd, item_emd], axis=0)
            ...
            cl_loss_train = self.get_ssl_con_loss(enc_embeds, prf_embeds, self.tau)
        ...
        # edit
        if cl_loss_train is not None:
            total_loss += cl_loss_train * self.beta
        # edit end
        return total_loss

    def calc_bpr_loss(
        self, user_emd, item_emd, user_list, pos_item_list, label#  neg_item_list
    ):
        ...
        l1 = torch.nn.MSELoss()(p_scores, label)
    ...
    def predict(self, interaction):
        if self.restore_user_e is None or self.restore_item_e is None:
            self.restore_user_e, self.restore_item_e, _ = self.forward(self.train_graph)
