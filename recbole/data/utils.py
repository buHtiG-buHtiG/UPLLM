...
def data_preparation_train(config, dataset):
    model_type = config["MODEL_TYPE"]
    built_datasets = dataset.build()
    ...
    if model_type != ModelType.KNOWLEDGE:
        train_data = get_dataloader(config, "evaluation")(
            config, train_dataset, train_sampler, shuffle=config["shuffle"]
        )
    else:
        kg_sampler = KGSampler(
...
def create_samplers(config, dataset, built_datasets):
    ...
    # print(train_neg_sample_args) {'distribution': 'none', 'sample_num': 'none', 'alpha': 'none', 'dynamic': False, 'candidate_num': 0}
    # print(eval_neg_sample_args) {'distribution': 'none', 'sample_num': 'none'}
    # time.sleep(1000)
    ...
    # print(train_sampler, valid_sampler, test_sampler)
    # time.sleep(1000)
