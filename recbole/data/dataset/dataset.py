...
# Below are imports in RecBole-GNN dataset.py
try:
...
class Dataset(torch.utils.data.Dataset):
    ...
    def _load_feat(self, filepath, source):
        ...
        if not os.path.exists("{}.pkl".format(filepath)):
            dfs = []
            with tqdm(desc='Reading CSV', unit='lines', unit_scale=True) as pbar:
            ...
            with open("{}.pkl".format(filepath), "rb") as f:
                df = pickle.load(f)
        df.columns = columns
        ...
        if source == FeatureSource.INTERACTION:
            df["rating_ori"] = df["rating"]
            self.field2type["rating_ori"] = self.field2type["rating"]
            self.field2source["rating_ori"] = self.field2source["rating"]
            self.field2seqlen["rating_ori"] = self.field2seqlen["rating"]
            self.field2bucketnum["rating_ori"] = self.field2type["rating"]
    ...
    def _get_remap_list(self, field_list):
        ...
        for field in field_list:
            # print(field, self.field2type, field_list)
            ftype = self.field2type[field]
            for feat in self.field2feats(field):
    ...
    # Below are functions in RecBole-GNN dataset.py
    @staticmethod
